...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
	...
	for i in symbol_quantity_list:
		sum += periodic_table_dict[i[0]][1] * i[1]
	return sum

def get_known_molecule(compound):
	...
	if compound in known_molecules_dict:
		return known_molecules_dict[compound]

	return compound

def main():
	user_molecule = input("Enter the molecular formula of the sample: ")
	user_mass = float(input("Enter the mass in grams of the sample: "))
	elements = make_periodic_table()
	user_formula = parse_formula(user_molecule, elements)
	total_mass = compute_molar_mass(user_formula, elements)
	moles = user_mass/total_mass
	print(f"Info for element: {get_known_molecule(user_molecule)}")
	print(f"{total_mass:.5f} grams/mole")
	print(f"{moles:.5f} moles")
	avogadro = 6.02214076e23
	print(f"{int(moles * avogadro)} molecules")
